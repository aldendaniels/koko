...
function init(e){ko=e,component.init(ko),kokoView.init(ko),_404.init(ko,component.createComponentViewModel),bindings.init(ko),setEarlyExports()}function setConfig(e){config=e,stateNode.init(ko,config,stateTree),stateTree.init(ko),urlResolver.init(ko,config);var o,t=[];for(o in config.routes)t.push(o);var r={};for(o in config.redirects){var i=config.redirects[o];r[o]="object"==typeof i?urlResolver.resolveAbsolutePathToUrl(i.path,i.params):urlResolver.resolveAbsolutePathToUrl(i)}router.start({rootUrl:config.rootUrl,routes:t,routeParams:config.routeParams,redirects:r,html5History:config.html5History||!1},onRoute),setLateExports()}function onRoute(e,o){e?stateTree.update(config.routes[e],o):stateTree.update(config.notFoundComponent||"kokoDefault404",o)}function navigateToPath(e,o,t){router.navigate(urlResolver.resolvePathToUrl(e,o,t))}function navigateToUrl(e){router.navigate(e)}function setEarlyExports(){utils.assign(module.exports,{componentViewModel:component.createComponentViewModel,config:setConfig})}function setLateExports(){utils.assign(module.exports,{resolve:urlResolver.resolvePathToUrl,navigateToPath:navigateToPath,navigateToUrl:navigateToUrl,root:{stateNode:stateTree.getRoot()}})}var router=require("./router"),stateNode=require("./state-node"),stateTree=require("./state-tree"),urlResolver=require("./url-resolver"),component=require("./component"),bindings=require("./bindings"),kokoView=require("./koko-view"),_404=require("./404"),utils=require("./utils"),ko,config;module.exports.init=init;
...
function normalizeUrlFragment(r){return r?("/"!==r.slice(0,1)&&(r="/"+r),"/"!==r.slice(r.length-1,1)&&(r+="/"),r):"/"}function startRouter(r,t){if(routes)throw new Error("The router is already started.");if(routes=r.routes,routeParams=r.routeParams,redirects=r.redirects,!routes||!routeParams||!redirects)throw new Error("Invalid configuration");var e,o=function(r){return r};for(e in r.routeParams)r.routeParams[e]=routeParams[e]||{},r.routeParams[e].parse=routeParams[e].parse||o,r.routeParams[e].regex=routeParams[e].regex||/(.*)/g;router=new director.Router,router.configure({html5history:r.html5History});for(e in routeParams)router.param(e,routeParams[e].regex);var a;for(var i in routes)a=routes[i],router.on(utils.optionalizeTrailingSlash(a),onRoute.bind(window,a,t));for(a in redirects)router.on(utils.optionalizeTrailingSlash(a),function(){navigate(redirects[a])});if(router.on(".*",onRoute.bind(window,"",t)),!router.history){var n=normalizeUrlFragment(window.location.pathname),u=normalizeUrlFragment(r.rootPath);if(n.slice(0,u.length)!==u)throw new Error('Path does not start with the provided "root" path');if(n=n.slice(u.length-1),"/"!==n)return void(window.location=u+"#"+n)}router.init(router.history?"":window.location.pathname)}function navigate(r){"/#"===r.slice(0,2)&&(r=r.slice(2)),router.setRoute(r)}function onRoute(r,t){var e=Array.prototype.slice.call(arguments,2),o={},a=0;utils.processPattern(r,function(r,t){t&&(o[r]=routeParams[r].parse(e[a]),a++)}),t(r,o)}var director=require("director"),utils=require("./utils"),router,routes,routeParams,redirects;module.exports={start:startRouter,navigate:navigate};
},{"./utils":13,"director":2}],10:[function(require,module,exports){
function init(t,i,n){ko=t,config=i,state=n}var utils=require("./utils"),ko,state,config,StateNode=utils.createClass({init:function(t,i,n){this.parent=n,this.component=t,this.children=ko.observableArray(),this.status=ko.observable("loading"),this.routeParams={};for(var e in config.routeParams)this.routeParams[e]=ko.observable(i[e]||null)},isVisible:function(){return"visible"===this.status()||"pending_removal"===this.status()},setReady:function(){"loading"===this.status()&&(this.status("pending_visible"),state.transitionIfReady())},updateRouteParams:function(t){if("pending_removal"===this.status())throw new Error("Params should not be updated while pending removal");var i;for(i in this.routeParams)i in t||this.routeParams[i](null);for(i in t){if(!(i in this.routeParams))throw new Error("Unrecognized parameter name.");this.routeParams[i](t[i])}},transition:function(){this.children.remove(function(t){return"pending_removal"===t.status()}),this.children().length&&(this.children()[0].status("visible"),this.children()[0].transition())},getPathToHere:function(){for(var t=[this.component],i=this;i.parent.component;)t.splice(0,0,i.parent.component),i=i.parent;return t.join(".")},loadChild:function(t){var i=this.getMatchingChild(t);if(i)return i;this.children.remove(function(t){return"loading"===t.status()}),this.children().length>1&&console.log("There should only be 1 active child component at a time"),this.children().forEach(function(t){t.status("pending_removal")});var n=new StateNode(t,ko.toJS(this.routeParams),this);return this.children.push(n),n},isBranchReady:function(){if("loading"===this.status())return!1;for(var t in this.children())if(!this.children()[t].isBranchReady())return!1;return!0},getMatchingChild:function(t){var i=this.children().filter(function(i){return i.component===t});return i.length?i[0]:null}});module.exports={init:init,StateNode:StateNode};
